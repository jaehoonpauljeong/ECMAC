...
void ChannelAccess::sendToChannel(cPacket *msg)
{
    ...
    if(useSendDirect){
        // use Andras stuff
    	if(chris){
    		std::cout << "gateList.size(): " << gateList.size() << endl;
    	}
...
simtime_t ChannelAccess::calculatePropagationDelay(const NicEntry* nic) {
	...
	// chris
	if(chris){
//		if ( strcmp(getParentModule()->getParentModule()->getFullName(), "node[41]" ) == 0 ){
//			std::cout << simTime() << " ### nic->chAccess: "<< &receiverModule << " : "<< static_cast<void*>(receiverModule) << endl;
//			std::cout << simTime() << " ### ";
			std::cout << getParentModule()->getParentModule()->getFullName() << endl;
			std::cout <<"         ===> "<< nic->nicPtr->getParentModule()->getFullName() << endl;
//		}
