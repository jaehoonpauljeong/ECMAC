...
static bool chris_decider = true;
...
simtime_t Decider80211p::processNewSignal(AirFrame* msg) {
	...
	if ( chris_decider ){
		DBG_D11P << "Decider80211p: signal, time: " << signal.getReceptionStart() << ", centerFrequency: " << centerFrequency << endl;
	}
	...
	if (recvPower < sensitivity) {
		if ( chris_decider )
			DBG_D11P << "recvPower < sensitivity: " << recvPower << " < " << sensitivity << endl;
...
//			curSyncFrame = 0;
			...
			DBG_D11P << "frame: " << frame->getId() << std::endl;
			if ( !curSyncFrame ) {
				...
				DBG_D11P << "curSyncFrame: " << curSyncFrame->getId() << std::endl;
			}
			else {
				...
				DBG_D11P << "curSyncFrame is not nullptr " << std::endl;
//				DBG_D11P << curSyncFrame->getName() << endl;
//				cout <<
			}
...
enum Decider80211p::PACKET_OK_RESULT Decider80211p::packetOk(double snirMin, double snrMin, int lengthMPDU, double bitrate) {
	...
	if ( chris_decider ){
		DBG_D11P << "DBG_D11P rand: " << rand << std::endl;
	}
	...
	if (!collectCollisionStats) {
		if (rand > headerNoError){
			...
			if ( chris_decider ){
				DBG_D11P << "rand > headerNoError " << headerNoError << std::endl;
			}
		...
		if (rand > headerNoError) {
			...
			if (rand > headerNoErrorSnr) {
				...
				if ( chris_decider ){
					DBG_D11P << "rand > headerNoErrorSnr " << headerNoErrorSnr << std::endl;
				}
	...
	if (!collectCollisionStats) {
		if (rand > packetOkSinr) {
			...
			if ( chris_decider ){
				DBG_D11P << "rand > packetOkSinr " << packetOkSinr << std::endl;
			}
		...
		if (rand > packetOkSinr) {
			...
			if (rand > packetOkSnr) {
				...
				if ( chris_decider ){
					DBG_D11P << "rand > packetOkSnr " << packetOkSnr << std::endl;
				}
...
simtime_t Decider80211p::processSignalEnd(AirFrame* msg) {
	...
	if (frame->getUnderSensitivity()) {
		...
		if (frame == curSyncFrame) {
			...
			DBG_D11P << "curSyncFrame: resetSyncFrame, frame: " << frame->getId() << endl;;
		}
		else {
	...
	if (phy11p->getRadioState() == Radio::TX) {
		...
		// Chris
		DBG_D11P << "Chris: Reset curSyncFrame to 0 " << endl;
		curSyncFrame = 0;
	}
