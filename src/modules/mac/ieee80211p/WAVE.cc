...
void Wave::initialize(int stage) {
...
void Wave::handleSelfMsg(cMessage* msg) {
...
void Wave::handleUpperControl(cMessage* msg) {
	assert(false);
}

void Wave::handleUpperMsg(cMessage* msg) {
...
void Wave::handleLowerControl(cMessage* msg) {
...
void Wave::handleLowerMsg(cMessage* msg) {
...
void Wave::setActiveChannel(t_channel state) {
	activeChannel = state;
...
void Wave::attachSignal(Mac80211Pkt* mac, simtime_t startTime, double frequency) {
...
Signal* Wave::createSignal(simtime_t start, simtime_t length, double power, double bitrate, double frequency) {
...
simtime_t Wave::timeLeftTillGuardOver() const {
	ASSERT(useSCH);
...
simtime_t Wave::timeLeftInSlot() const {
	ASSERT(useSCH);
...
void Wave::changeServiceChannel(int cN) {
	ASSERT(useSCH);
...
int Wave::EDCA::queuePacket(t_access_category ac,CMACWSM* msg) {
...
int Wave::EDCA::createQueue(int aifsn, int cwMin, int cwMax,t_access_category ac) {
...
Wave::t_access_category Wave::mapPriority(int prio) {
...
CMACWSM* Wave::EDCA::initiateTransmit(simtime_t lastIdle) {
...
simtime_t Wave::EDCA::startContent(simtime_t idleSince,bool guardActive) {
...
void Wave::EDCA::stopContent(bool allowBackoff, bool generateTxOp) {
	//update all Queues
...
void Wave::EDCA::backoff(t_access_category ac) {
...
void Wave::EDCA::postTransmit(t_access_category ac) {
...
void Wave::channelBusySelf(bool generateTxOp) {
...
void Wave::channelIdle(bool afterSwitch) {
...
void Wave::setParametersForBitrate(int bitrate) {
...
simtime_t Wave::getFrameDuration(int payloadLengthBits) const {
