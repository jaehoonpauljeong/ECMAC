...
//const simsignalwrap_t CMac1609_4::roundCountSignal = simsignalwrap_t(MIXIM_SIGNAL_ROUND);
const simsignalwrap_t CMac1609_4::mobilityStateChangedSignal = simsignalwrap_t(MIXIM_SIGNAL_MOBILITY_CHANGE_NAME);



#include "modules/mobility/traci/TraCIScenarioManagerLaunchd.h"
#include "modules/application/traci/CMACApp11p.h"

using Veins::TraCIScenarioManagerLaunchd;


Define_Module(CMac1609_4);
...
void CMac1609_4::initialize(int stage) {
...
//                     std::cout << fileN.substr(pos1+5, pos2-(pos1+5)) << endl;
        ...
        if (FindModule<CMACApp11p*>::findGlobalModule()){
            CMACApp11p* appL = FindModule<CMACApp11p*>::findGlobalModule();
...
void CMac1609_4::handleSelfMsg(cMessage* msg) {
    if (msg == nextChannelSwitch) {
		...
		std::string vehMinOffset = "";
		for (auto i:vehs_Map){
			Veins::TraCIMobility* iMob;
			iMob = (Veins::TraCIMobility*)i.second->getSubmodule("veinsmobility");
			double offset = iMob->getCommandInterface()->getLanePosition(iMob->getExternalId());
	    	if ( minOffset == 0.0 ){
	    		minOffset = offset;
	    		vehMinOffset = i.second->getFullName();
	    	}
    	...
    	else if ( myNodeID.compare(vehMinOffset) == 0 ){
			scheduleAt(simTime(), nextClusteringEvent);
...
/*debugging end*/
		...
		std::sort( vehs_Vec.begin(), vehs_Vec.end(), cmpFunction_cmac );
...
/*debugging end*/
		...
		for (itVehVec = vehs_Vec.begin(); itVehVec != vehs_Vec.end(); itVehVec++ ){
			...
			for (itVehVec2 = vehs_Vec.begin(); itVehVec2 != vehs_Vec.end(); itVehVec2++ ){
				...
				CMac1609_4* cmac1609_4_2 = check_and_cast<CMac1609_4*>(macModuel2);
...
//				cModule* mobModuleCH = moduleMinAveDis->getModuleByPath(".veinsmobility");
				CMac1609_4* cmac1609_4CH = check_and_cast<CMac1609_4*>(macModuleCH);
...
/*debugging end*/
				...
				for (unsigned int i = 0; i < cmac1609_4CH->mClusterNeighborsVec.size(); i++){
					...
					CMac1609_4* cmac1609_4CM = check_and_cast<CMac1609_4*>(macModuleCM);
...
/*debug*/
		...
		if ( myOptimalLayer > 2){
			...
			CMac1609_4* cmac1609_4MCH = check_and_cast<CMac1609_4*>(macModuleMCH);
...
//				CMac1609_4* cmac1609_4CoveredCH = check_and_cast<CMac1609_4*>(macModuleCoveredCH);
...
void CMac1609_4::handleUpperControl(cMessage* msg) {
    assert(false);
}

void CMac1609_4::handleUpperMsg(cMessage* msg) {
...
void CMac1609_4::handleLowerControl(cMessage* msg) {
...
void CMac1609_4::handleLowerMsg(cMessage* msg) {
...
void CMac1609_4::setActiveChannel(t_channel state) {
    activeChannel = state;
    assert(state == type_CCH || (useSCH && state == type_SCH));
}

void CMac1609_4::finish() {
    //clean up queues.
...
void CMac1609_4::attachSignal(CMac80211Pkt* mac, simtime_t startTime, double frequency) {
...
Signal* CMac1609_4::createSignal(simtime_t start, simtime_t length, double power, double bitrate, double frequency) {
...
bool CMac1609_4::guardActive() const {
...
simtime_t CMac1609_4::timeLeftTillGuardOver() const {
    ASSERT(useSCH);
...
simtime_t CMac1609_4::timeLeftInSlot() const {
    ASSERT(useSCH);
...
void CMac1609_4::changeServiceChannel(int cN) {
    ASSERT(useSCH);
...
int CMac1609_4::EDCA::queuePacket(t_access_category ac,CMACWSM* msg) {
...
int CMac1609_4::EDCA::createQueue(int aifsn, int cwMin, int cwMax,t_access_category ac) {
...
CMac1609_4::t_access_category CMac1609_4::mapPriority(int prio) {
...
CMACWSM* CMac1609_4::EDCA::initiateTransmit(simtime_t lastIdle) {
...
simtime_t CMac1609_4::EDCA::startContent(simtime_t idleSince,bool guardActive) {
...
void CMac1609_4::EDCA::stopContent(bool allowBackoff, bool generateTxOp) {
    //update all Queues
...
void CMac1609_4::EDCA::backoff(t_access_category ac) {
...
void CMac1609_4::EDCA::postTransmit(t_access_category ac) {
...
void CMac1609_4::EDCA::revokeTxOPs() {
    for (std::map<t_access_category, EDCAQueue>::iterator iter = myQueues.begin(); iter != myQueues.end(); iter++) {
...
void CMac1609_4::channelBusySelf(bool generateTxOp) {
...
void CMac1609_4::channelIdle(bool afterSwitch) {
...
void CMac1609_4::setParametersForBitrate(int bitrate) {
...
simtime_t CMac1609_4::getFrameDuration(int payloadLengthBits) const {
...
bool cmpFunction_cmac(cModule* i, cModule* j){
	Veins::TraCIMobility* i_Mob;
...
int CMac1609_4::optTreeDepth( int numNodes ){
    int optd;
...
int CMac1609_4::optNumChildren( int numNodes, int optd){
//    return (int)(pow(numNodes,(1/(double)optd))+1);
    return (int)(pow(numNodes,(1/(double)optd)));
}

/*this function is not used*/
cModule* CMac1609_4::findCenter(cModule* currentVeh,
								CMac1609_4* cmac1609_4,
								float& minAveDis,
...
void CMac1609_4::sendTAF(TAF* taf) {
    //we actually came to the point where we can send a packet
    channelBusySelf(true);
//    CMACWSM* pktToSend = myEDCA[activeChannel]->initiateTransmit(lastIdle);
...
void CMac1609_4::sendUnicastPacket(cPacket* msg, int destAddr){
    //we actually came to the point where we can send a packet
    channelBusySelf(true);
//    CMACWSM* pktToSend = myEDCA[activeChannel]->initiateTransmit(lastIdle);
...
void CMac1609_4::sendBroadcastPacket(cPacket* msg){
    //we actually came to the point where we can send a packet
    channelBusySelf(true);
...
CMACWSM* CMac1609_4::prepareUpload(){
...
CHtoCH* CMac1609_4::prepareCHtoCH(){
...
CHDistr* CMac1609_4::prepareCHDistr(){
...
CMACWSM* CMac1609_4::getPktFromQ(){
...
CMACWSM* CMac1609_4::getPktFromQForCH2CH(){
	CMACWSM* tmpwsm = nullptr;
...
CMACWSM* CMac1609_4::getPktFromQForCHDistr(){
	CMACWSM* tmpwsm;
...
void CMac1609_4::clusterChannelAssign(std::vector<cModule*> nodeSet,
														int numNodes,
...
CMac1609_4::timeSlotAllocation(std::vector<cModule*> nodeSet,
                                                 int clusterNum,
...
vector< vector<cModule*> >
CMac1609_4::timeSlotSequence(std::vector<cModule*> nodeSet,
                                                 int clusterNum,
                                                 int optimalClusterSize){
...
void CMac1609_4::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj) {
	Enter_Method_Silent();
...
double CMac1609_4::getOffsetRatio() {
...
double CMac1609_4::getOffsetRatio(Veins::TraCIMobility* mob) {
...
void CMac1609_4::searchChannel(	int numClusters,
								int numAvailChannels,
								int max_cl,
								queue<int>& remain_channel_q,
								int ClusterChannel[]){
