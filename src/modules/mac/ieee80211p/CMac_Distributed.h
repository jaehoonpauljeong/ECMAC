...
#include "ACKPkt_m.h"
#include "InitiatingPkt_m.h"
#include "DeclareCHPkt_m.h"
#include "JoinPkt_m.h"
#include "CMConfirmPkt_m.h"

// PCF InterFrame Space
#define PIFS SIFS_11P + SLOTLENGTH_11P	// 32us + 13us = 45us
// DCF InterFrame Space for Contention Period
//#define DIFS SIFS_11P + (SLOTLENGTH_11P*2) // 32us + 13us*2 = 58us
 ...
 * @see Decider80211p
 */
class CMac_Distributed : public BaseMacLayer,
	...
	public:
		...
		void setRoleInCluster(t_role_in_cluster);

		cModule* findCenter(cModule*, CMac_Distributed*, float& );

		TAF* prepareTAF();

		simtime_t sendPacket(cPacket*);

		simtime_t sendPacketUnicast(cPacket*, int);
	...
	protected:
		...
		/*Events and messages*/
		cMessage* nextInitialEvent;
		cMessage* nextInitiationEvent;
		cMessage* nextFormClusterEvent;
		cMessage* nextDeclareCHEvent;
		cMessage* nextJoinEvent;
		...
		cMessage* nextTimeSlotAssignment;
		cMessage* nextCMComfirm;
		...
		std::map<int,CMMobility> mCMMobilityMap;

		t_role_in_cluster mRoleInCluster;

		cModule* mClusterHead;
		std::string mCH;
		int mCHMacAdd;
		Coord mCHCoord;
