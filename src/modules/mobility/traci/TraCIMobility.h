...
//#include "modules/mobility/traci/TMACScenarioManager.h"
...
class TraCIMobility : public BaseMobility
{
	public:
		...
		// Chris
		double commandGetOffset(){
			return getCommandInterface()->getLanePosition(getExternalId());
		}

		Coord getSumoPosition(){
			Coord Pos;
			TraCICoord sumoPos;
			sumoPos = getManager()->omnet2traci(getCurrentPosition());
			Pos.x = sumoPos.x;
			Pos.y = sumoPos.y;
			return Pos;
		}

		// return route Id, not edge ids in the route
		std::list<std::string> getRouteIdList(){
			return getCommandInterface()->getRouteIds();
		}

		std::list<std::string> getPlannedEdgeIdList(){
			return getCommandInterface()->getPlannedEdgeIds(getExternalId());
		}

		std::string getCurrentEdgeId(){
			return getCommandInterface()->getEdgeId(getExternalId());
		}
		// Chris
