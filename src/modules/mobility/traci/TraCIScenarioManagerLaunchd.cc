...
void TraCIScenarioManagerLaunchd::initialize(int stage)
{
	...
	// SCMAC, for randomize simulation, Chris
//	std::cout << launchConfig->detailedInfo() << endl<< endl;
//	launchConfig->getChildrenByTagName("seed");
//	cXMLElement* seedXML = launchConfig->getLastChild();
//	std::stringstream sss; sss << seed;
//	seedXML->setAttribute("value",sss.str().c_str());
//	std::cout << launchConfig->detailedInfo() << endl;
	// SCMAC
