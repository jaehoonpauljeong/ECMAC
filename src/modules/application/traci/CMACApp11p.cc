...
#include "application/traci/CMACApp11p.h"

using Veins::TraCIMobilityAccess;
using Veins::AnnotationManagerAccess;

const simsignalwrap_t CMACApp11p::parkingStateChangedSignal = simsignalwrap_t(TRACI_SIGNAL_PARKING_CHANGE_NAME);

Define_Module(CMACApp11p);
...
void CMACApp11p::initialize(int stage) {
...
void CMACApp11p::handleSelfMsg(cMessage* msg){
	switch (msg->getKind()) {
                ...
                if ( getOffsetRatio(traci) < 0.98 && getOffsetRatio(traci) > 0.85){
                    ...
                    for (it=allVeh.begin(); it!=allVeh.end(); ++it ){
                        ...
                        if (gLatestNodeId.empty()){
                            if (getOffsetRatio(i_Mob) < 0.3){
                                gLatestNodeId = it->second->getFullName();
                            }
                        }
                    }
...
//                    std::cout << "farthestNodeId: " << farthestNodeId << endl;
                    ...
                    if ( farthestNodeId.compare(findHost()->getFullName()) == 0){
...
void CMACApp11p::handleLowerMsg(cMessage* msg){
...
void CMACApp11p::onBeacon(CMACWSM* wsm) {
	delete(wsm);
}

void CMACApp11p::onData(CMACWSM* wsm) {
...
void CMACApp11p::onStatus(CMACWSM* wsm) {
//	findHost()->getDisplayString().updateWith("r=8,green");
//	annotations->scheduleErase(1, annotations->drawLine(wsm->getSenderPos(), traci->getPositionAt(simTime()), "blue"));
	gPeTotalRxPkts++;
	if ( simTime() < (wsm->getTimestamp() + wsm->getTTL()) ){
...
void CMACApp11p::sendMessage(std::string blockedRoadId) {
	sentMessage = true;
...
void CMACApp11p::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj) {
	Enter_Method_Silent();
...
void CMACApp11p::handleParkingUpdate(cObject* obj) {
...
void CMACApp11p::handlePositionUpdate(cObject* obj) {
...
void CMACApp11p::sendWSM(CMACWSM* wsm) {
//	if (isParking && !sendWhileParking) return;
	sendDown(wsm);
}

CMACWSM* CMACApp11p::prepareWSM(std::string name,
								int lengthBits,
								t_channel channel,
								int priority,
								int rcvId,
...
/*debug*/
	...
	*/
	...
	for ( std::vector<cModule*>::iterator itt = allVehNode.begin(); itt != allVehNode.end(); ++itt ){
        ...
        CMACApp11p* cmacApp = check_and_cast<CMACApp11p*>(appModule);
...
double CMACApp11p::getOffsetRatio(Veins::TraCIMobility* vehMob) {
